...
Đây là cấu hình phù hợp cho Checkpoint 2/pilot nhỏ: một URL, một cold start và không cần Blueprint hay Private Service. Khi chạy production nhiều instance, mới nên tách Web, API và AI thành ba service riêng.
...
Không cần đặt `PORT`, `API_PORT`, `AI_SERVICE_URL`, `API_INTERNAL_URL` hoặc `NEXT_PUBLIC_API_URL`:
...
Nếu có khóa TTS riêng, thêm `TTS_API_KEY`. Nếu không, server dùng `EXTERNAL_LLM_API_KEY`.
...
3. URL gốc phải mở màn hình đăng nhập.
4. Đăng nhập tài khoản pilot đang tồn tại trên Supabase.
5. Học sinh: khóa học → Practice → nộp câu → xem recommendation, model version, signals và reasons.
6. Giảng viên: chọn nguồn `VERIFIED` → tạo draft → xác nhận provider `EXTERNAL_LLM` → sửa → gửi duyệt → approve → publish.
7. Quay lại phía học sinh để xác nhận chỉ nội dung `PUBLISHED` mới xuất hiện.
