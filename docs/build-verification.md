...
| `npm audit --omit=dev --audit-level=high` | PASS | 2 moderate từ PostCSS do Next 16.2.10 pin; không có high/critical |
...
`npm audit --omit=dev` hiện báo hai finding moderate cùng gốc ở PostCSS 8.4.31 do Next 16.2.10 pin. `npm audit fix --force` đề xuất hạ về Next 9.3.3 nên không được áp dụng. Sản phẩm không nhận CSS từ người dùng/provider, giúp thu hẹp đường khai thác, nhưng advisory vẫn được công khai. GitHub Actions fail ở mức high/critical và tiếp tục hiển thị moderate để nâng cấp khi upstream có bản vá tương thích.
